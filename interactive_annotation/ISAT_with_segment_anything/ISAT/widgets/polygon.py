...
class Vertex(QtWidgets.QGraphicsPathItem):
    ...
    def itemChange(self, change: 'QtWidgets.QGraphicsItem.GraphicsItemChange', value: typing.Any):
        ...
        if change == QtWidgets.QGraphicsItem.GraphicsItemChange.ItemPositionChange and self.isEnabled():
            ...
            # clean previous polygon mask
            has_moved = False
            if self.polygon.points[index] != value:
                has_moved = True
            if has_moved and self.scene().current_graph is None and self.scene().mask_image_data is not None:
                self.scene().update_progress(self.polygon.points, 0, False)

            # update polygon points
            self.polygon.movePoint(index, value)

            # update polygon mask with moved points
            if has_moved and self.scene().current_graph is None and self.scene().mask_image_data is not None:
                self.scene().update_progress(self.polygon.points, 255, False)
                self.scene().is_point_moved = True

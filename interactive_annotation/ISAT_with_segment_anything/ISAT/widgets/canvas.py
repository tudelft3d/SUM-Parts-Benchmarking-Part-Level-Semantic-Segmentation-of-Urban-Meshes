...
class AnnotationScene(QtWidgets.QGraphicsScene):
    def __init__(self, mainwindow):
        ...
        self.pressd = False
        self.is_point_moved = False

        self.p_start_time = time.time()
        self.basic_operation_num = 0
        self.smart_operation_num = 0
    ...
    def load_image(self, image_path:str):
        ...
        self.mask_image_data = np.zeros(self.image_data.shape[:2], np.uint8)
    ...
    def finish_draw(self):
        ...
        if self.draw_mode == DRAWMode.SEGMENTANYTHING:
            ...
            if self.masks is not None:
                ...
                self.smart_operation_num += len(self.prompt_points)
                ...
                for index, contour in enumerate(contours):
                    ...
                    # update progress
                    self.update_progress(self.current_graph.points)
            ...
            if len(self.current_graph.points) < 1:
                return
            self.basic_operation_num += len(self.current_graph.points)
    ...
    def delete_selected_graph(self):
        ...
        for item in self.selectedItems():
            if isinstance(item, Polygon) and (item in self.mainwindow.polygons):
                ...
                # update progress
                self.update_progress(item.points, 0)
    ...
    def mouseReleaseEvent(self, event: 'QtWidgets.QGraphicsSceneMouseEvent'):       
        self.pressd = False
        if self.is_point_moved:
            self.basic_operation_num += 1
            self.is_point_moved = False
    ...
    def update_progress(self, points, update_value = 255, write_file = True):
        polygon_points = np.array([(p.x(), p.y()) for p in points], np.int32)
        polygon_points = polygon_points.reshape((-1, 1, 2))
        poly_mask = np.zeros(self.mask_image_data.shape[:2], np.uint8)
        cv2.fillPoly(poly_mask, [polygon_points], 255)
        self.mask_image_data = np.where(poly_mask == 255, update_value, self.mask_image_data)
        self.mainwindow.info_dock_widget.update_progress_bar(write_file)

...
# ----------------------------------------------------------------------------------------------------------------------
#
#      Class handling UrbanMesh dataset.
...
def read_ply_with_plyfilelib(filename):
    ...
    plydata = PlyData.read(filename)
    xyz = np.stack([plydata['vertex'][n] for n in ['x', 'y', 'z']], axis=1)
    normals = np.stack([plydata['vertex'][n] for n in ['nx', 'ny', 'nz']], axis=1)
    try:
        rgb = np.stack([plydata['vertex'][n]
                        for n in ['red', 'green', 'blue']]
                       , axis=1).astype(np.uint8)
    ...
    try:
        object_indices = plydata['vertex']['object_index']
        labels = plydata['vertex']['label']
        return xyz, normals, rgb, labels, object_indices
    except ValueError:
        try:
            labels = plydata['vertex']['label']
            return xyz, normals, rgb, labels
        except ValueError:
            return xyz, normals, rgb


def write_ply_with_plyfilelib(filename, xyz, normals, rgb, label):
    """write into a ply file"""
    prop = [('x', 'f8'), ('y', 'f8'), ('z', 'f8'), ('nx', 'f8'), ('ny', 'f8'), ('nz', 'f8'), ('red', 'u1'), ('green', 'u1'), ('blue', 'u1'), ('label', 'u1')] #Classification',
    #colors = COLOR_MAP[np.asarray(label)]
    vertex_all = np.empty(len(xyz), dtype=prop)
    for i_prop in range(0, 3):
        vertex_all[prop[i_prop][0]] = xyz[:, i_prop]
    ...
    for i_prop in range(0, 3):
        vertex_all[prop[i_prop + 6][0]] = rgb[:, i_prop]
    vertex_all[prop[9][0]] = label
    ply = PlyData([PlyElement.describe(vertex_all, 'vertex')], text=False) # True ascii
    ply.write(filename)
...
class UrbanMeshDataset(PointCloudDataset):
    """Class to handle UrbanMesh dataset."""

    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        """
        This dataset is small enough to be stored in-memory, so load all point clouds here
        """
        PointCloudDataset.__init__(self, 'UrbanMesh')
        ...
        # self.label_to_names = {0: 'unlabelled',
        #                        1: 'ground',
        #                        2: 'vegetation',
        #                        3: 'building',
        #                        4: 'water',
        ...
        # Dataset folder
        self.root = ROOT_DIR + '/'
        self.path = ROOT_DIR + '/data/sumv2_tri_demo/'
        ...
        if self.set == 'training':
            ...
            raise ValueError('Unknown set for UrbanMesh data: ', self.set)
        ...
        for folder in folders:
            data_folder = self.path + folder
            #data_folder = self.path + "/raw_small/" + folder
            files = glob.glob(data_folder + "*.ply")
            if self.set == 'training' and folder == "train/":
                self.files += files
                for file in files:
                    file_name = os.path.splitext(os.path.basename(file))[0]
                    self.cloud_names.append(file_name)
                break
            elif self.set == 'validation' and folder == "val/":
                self.files += files
                for file in files:
                    file_name = os.path.splitext(os.path.basename(file))[0]
                    self.cloud_names.append(file_name)
                break
            elif self.set == 'test' and folder == "test/":
                self.files += files
                for file in files:
                    file_name = os.path.splitext(os.path.basename(file))[0]
                    self.cloud_names.append(file_name)
    ...
    def potential_item(self, batch_i, debug_workers=False):
        ...
        while True:
            ...
            input_normals = self.input_normals[cloud_ind][input_inds]
            ...
            input_features = np.hstack((input_normals, input_colors)).astype(np.float32)
    ...
    def load_subsampled_clouds(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                data, sub_normals, sub_colors, sub_labels = read_ply_with_plyfilelib(sub_ply_file)
                ...
                # Read ply file
                xyz, normals, rgb, rawlabels = read_ply_with_plyfilelib(file_path)
                points = xyz
                colors = 255 * rgb  # Now scale by 255
                ...
                feats = np.hstack((normals, colors))

                # Subsample cloud
                sub_points, sub_feats, sub_labels = grid_subsampling(points,
                                                                      features=feats,
                                                                      labels=labels,
                                                                      sampleDl=dl)

                # Rescale float color and squeeze
                sub_normals, sub_colors = np.split(sub_feats, 2, axis=1)
                sub_colors = sub_colors / 255
                sub_labels = np.squeeze(sub_labels)

                # Get chosen neighborhoods
                search_tree = KDTree(sub_points, leaf_size=50) #10
                ...
                write_ply_with_plyfilelib(sub_ply_file, sub_points, sub_normals, sub_colors, sub_labels)

            # Fill data containers
            self.input_trees += [search_tree]
            self.input_normals += [sub_normals]
        ...
        if self.set in ['validation', 'test']:
            ...
            for i, file_path in enumerate(self.files):
                ...
                if exists(proj_file):
                    ...
                    # Read ply file
                    xyz, normal, rgb, rawlabels = read_ply_with_plyfilelib(file_path)
                    points = xyz
                    colors = 255 * rgb  # Now scale by 255
                    labels = np.array([x if x > 0 else x + 1 for x in rawlabels])
...
class UrbanMeshSampler(Sampler):
    """Sampler for UrbanMesh"""

    def __init__(self, dataset: UrbanMeshDataset):
...
class UrbanMeshCustomBatch:
    """Custom batch definition with memory pinning for UrbanMesh"""
    ...
    def __init__(self, input_list):
        ...
        ind += 1
        self.labels = torch.from_numpy(input_list[ind]).long() # torch.from_numpy(input_list[ind])
        ind += 1
...
def UrbanMeshCollate(batch_data):
    return UrbanMeshCustomBatch(batch_data)

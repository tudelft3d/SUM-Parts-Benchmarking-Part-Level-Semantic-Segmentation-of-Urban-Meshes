...
def main():
    ...
    #'Weight decay' for avoiding overfiting, like regularization, 0: no drop out, 1: too much drop out and larger loss value
    # for get optimized value
    ####################Change param##########################default=0
    parser.add_argument('--wd', default=0, type=float, help='Weight decay')
    ####################Change param##########################default=1e-2, opt 1e-3
    parser.add_argument('--lr', default=1e-3, type=float, help='Initial learning rate')
    ####################Change param##########################default=0.7
    ...
    #***********#####Change param##########################Empty for train(''), testing('RESUME')
    ...
    ####################Change param##########################default='gru_10,f_10', 10 classes, f_classes, see graphnet.py for 'gru'
    # matrix-vector gru10, vector-vector gru10_0
    # Model,gru_10, f_class, be careful, #default='gru_10,f_6', H3D: f_11, S3DIS: f_13
    # using: gru_10_1_1_1_0, nrepeats, vv, layernorm, ingate, cat_all, #gru_10_0_1_1_1 not working
    parser.add_argument('--model_config', default='gru_10_1_1_1_0,f_6',
    ...
    ####################Change param, no space in string##########################
    #if use point cloud features
    ...
    #if use mesh features use_mesh_features
    #'delta_avg,delta_std,linearity/ld,verticality/ld,curvature/ld,sphericity/ld,planarity/ld,xyz/ld,vcount/ld,triangle_density/ld,ele/ld,inmatrad/ld,circumference/ld,shape_descriptor/ld,compactness/ld,shape_index/ld,muls2_ele_0/ld,muls2_ele_1/ld,muls2_ele_2/ld,pt2plane_dist_mean/ld,red/ld,green/ld,blue/ld,hue/ld,sat/ld,val/ld,hue_var/ld,sat_var/ld,val_var/ld,greenness/ld,hue_bin_0/ld,hue_bin_1/ld,hue_bin_2/ld,hue_bin_3/ld,hue_bin_4/ld,hue_bin_5/ld,hue_bin_6/ld,hue_bin_7/ld,hue_bin_8/ld,hue_bin_9/ld,hue_bin_10/ld,hue_bin_11/ld,hue_bin_12/ld,hue_bin_13/ld,hue_bin_14/ld,sat_bin_0/ld,sat_bin_1/ld,sat_bin_2/ld,sat_bin_3/ld,sat_bin_4/ld,val_bin_0/ld,val_bin_1/ld,val_bin_2/ld,val_bin_3/ld,val_bin_4/ld'
    ...
    ####################Change param########################## default=''xyzrgbelpsvXYZ' 'xyzrgbelpsv' 'xyzrgbvpsaec'
    # if use point cloud features
    # parser.add_argument('--pc_attribs', default='xyzrgbelpsv',
    ...
    # if use mesh features
    # default='xyzrgball', !!!change here need to update load_superpoint() in my_spg and --ptn_nfeat_stn
    # new default = xyzrgball
    ...
    ####################Change param########################## default = 0, can be 1.2
    ...
    ####################Change param########################## default=0, can be 0.2
    ...
    # Point net
    ####################Change param########################## default=40, or 3 vertex + face ceneter = 4
    #new default = 10
    ...
    # best test: '[[64,64,128,128,256],[256,128,128,64,64]]'
    # use global no pointnet '[[64,64,128,128,256],[ptn_nfeat_global]]'
    ...
    ####################Change param##########################
    #point-based features: point-based color features + linearity, verticality, curvature, sphericity, planarity: 9 + 5
    #mesh features default xyz all = 67 + 6(xyzrgb) = 73, 52 + 6 = 58,  #3 + 3 = 6(xyzrgb) or 6 + 9 + 5 = 20
    parser.add_argument('--ptn_nfeat_stn', default=6, type=int,
                        help='PointNet\'s Transformer number of input features')
    #add global features, 53 + 1 (diameter) = 54 - 2 (circum, norm_z_var) - 4 (ele*3, relele) = 48, 53 #1,
    ...
    if args.dataset == 'custom_dataset':
        # import my_custom_dataset  # <- to write!
        # dbinfo = my_custom_dataset.get_info(args)
        # create_dataset = my_custom_dataset.get_datasets
        import my_custom_dataset_myg  # <- to write!
        dbinfo = my_custom_dataset_myg.get_info(args)
        create_dataset = my_custom_dataset_myg.get_datasets
    else:
    ...
    #print('Train dataset: %i elements - Test dataset: %i elements' % (len(train_dataset), len(test_dataset)))
    ...
    def train():
        ...
        loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, collate_fn=my_spg.eccpc_collate,
    ...
    def eval(is_valid=False):
        ...
        if is_valid:  # validation
            loader = torch.utils.data.DataLoader(valid_dataset, batch_size=args.batch_size, collate_fn=my_spg.eccpc_collate,
                                                 num_workers=args.nworkers)
        else:  # evaluation
            loader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, collate_fn=my_spg.eccpc_collate,
                                                 num_workers=args.nworkers)
    ...
    def eval_final():
        ...
        for ss in range(args.test_multisamp_n):
            ...
            loader = torch.utils.data.DataLoader(test_dataset_ss, batch_size=1, collate_fn=my_spg.eccpc_collate,
                                                 num_workers=args.nworkers)
            if logging.getLogger().getEffectiveLevel() > logging.DEBUG: loader = tqdm(loader, ncols=65)

...
def compute_sp_graph(xyz, d_max, in_component, components, labels, n_labels):
    ...
    for i_com in range(0, n_com):
        ...
        graph["sp_point_count"][i_com] = len(comp)#->vertex count
        ...
        if len(xyz_sp) == 1:
            ...
            try:
                ...
                graph["sp_length"][i_com] = 0
            try:
                graph["sp_surface"][i_com] = np.sqrt(ev[0] * ev[1] + 1e-10)#->plane fitting score(planarity)
            except TypeError:
                graph["sp_surface"][i_com] = 0
            try:
                graph["sp_volume"][i_com] = np.sqrt(ev[0] * ev[1] * ev[2] + 1e-10)#->change to segment area
            except TypeError:
                graph["sp_volume"][i_com] = 0

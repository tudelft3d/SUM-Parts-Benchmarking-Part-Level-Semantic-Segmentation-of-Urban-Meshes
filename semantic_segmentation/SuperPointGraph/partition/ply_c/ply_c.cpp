...
{//compute the following geometric features (geof) features of a point cloud:
 //linearity planarity scattering verticality
    ...
    {//each point can be treated in parallell independently
        ...
        float scattering =  sqrtf(lambda[2]) / sqrtf(lambda[0]);//= sphericity
		//------Weixiao Update-------//
		float curvature = 3.0f * sqrtf(lambda[2]) / (sqrtf(lambda[0]) + sqrtf(lambda[1]) + sqrtf(lambda[2]));
		//---------------------------//
		...
		geof[i_ver][0] = linearity; //sqrtf(lambda[0]);// linearity;//curvature; //
		geof[i_ver][1] = planarity;// sqrtf(lambda[1]);
		geof[i_ver][2] = scattering;//sqrtf(lambda[2]);
		geof[i_ver][3] = curvature;//sqrtf(lambda[2]);

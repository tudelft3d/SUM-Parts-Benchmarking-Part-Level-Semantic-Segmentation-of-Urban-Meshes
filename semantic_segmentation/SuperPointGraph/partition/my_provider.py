...
def get_color_from_label(object_label, dataset):
    ...
    if dataset == 's3dis':  # S3DIS
        ...
        #     8: [78,78,75],     # table
        #     9: [41,44,104],     # chair
        #     10: [217,49,50],   # bookcase
        #     11: [87,40,96],     # sofa
        #     12: [85,109,115],     # board
        #     13: [234,234,230],     # clutter
        # }.get(object_label, -1)
        ...
        if voxel_width > 0:
            if has_labels > 0:
                ...
                xyz_sub, rgb_sub, l, o = libply_c.prune(xyz_full, voxel_width
                                                        , rgb_full, np.zeros(1, dtype='uint8'),
...
def read_las(filename):
    ...
    try:
        ...
        raise ValueError(
            "laspy package not found. uncomment import in /partition/provider and make sure it is installed in your environment")
    N_points = len(inFile)
...
def read_aug_graph_ply(filename):
    """convert from a ply file. include the label and the object number"""
    # ---read the ply file--------
    plydata = PlyData.read(filename)
    try:
        xyz = np.stack([plydata['vertex'][n] for n in ['x', 'y', 'z']], axis=1)
        edges = np.stack([plydata['edge'][n] for n in ['vertex1', 'vertex2']], axis=1)
        center_offsets = np.stack([plydata['edge'][n] for n in ['center_offset_x', 'center_offset_y', 'center_offset_z']], axis=1)
        return xyz, edges, center_offsets
    except ValueError:
        return xyz
# ------------------------------------------------------------------------------
...
def write_my_features(file_name, geof, xyz, rgb, labels):
    """write the geometric features, labels and clouds in a h5 file"""
    if os.path.isfile(file_name):
        os.remove(file_name)
    data_file = h5py.File(file_name, 'w')
...
def read_my_features(file_name):
    """read the geometric features, clouds and labels from a h5 file"""
    data_file = h5py.File(file_name, 'r')
    # fist get the number of vertices
    n_ver = len(data_file["geof"][:, 0])
...
def write_spg(file_name, graph_sp, components, in_component):
    ...
    # data_file.create_dataset('se_verticality_ratio'
    #                          , data=graph_sp["se_verticality_ratio"], dtype='float32')
    # data_file.create_dataset('se_planarity_ratio'
    #                          , data=graph_sp["se_planarity_ratio"], dtype='float32')
    # data_file.create_dataset('se_area_ratio'
    #                          , data=graph_sp["se_area_ratio"], dtype='float32')
    # data_file.create_dataset('se_ele_ratio'
    #                          , data=graph_sp["se_ele_ratio"], dtype='float32')
    # data_file.create_dataset('se_matrad_ratio'
    #                          , data=graph_sp["se_matrad_ratio"], dtype='float32')
    # data_file.create_dataset('se_point_count_ratio'
    #                          , data=graph_sp["se_point_count_ratio"], dtype='float32')

def write_my_spg(file_name, graph_sp, components, in_component):
    """save the partition and spg information"""
    if os.path.isfile(file_name):
        os.remove(file_name)
    data_file = h5py.File(file_name, 'w')
    ...
    data_file.create_dataset('sp_ele'
                             , data=graph_sp["sp_ele"], dtype='float32')
    data_file.create_dataset('sp_inmat_rad'
                             , data=graph_sp["sp_inmat_rad"], dtype='float32')
    data_file.create_dataset('sp_circumference'
                             , data=graph_sp["sp_circumference"], dtype='float32')
    ...
    data_file.create_dataset('sp_muls2_ele_0'
                             , data=graph_sp["sp_muls2_ele_0"], dtype='float32')
    data_file.create_dataset('sp_muls2_ele_1'
                             , data=graph_sp["sp_muls2_ele_1"], dtype='float32')
    data_file.create_dataset('sp_muls2_ele_2'
                             , data=graph_sp["sp_muls2_ele_2"], dtype='float32')
    data_file.create_dataset('sp_pt2plane_dist_mean'
                             , data=graph_sp["sp_pt2plane_dist_mean"], dtype='float32')
    data_file.create_dataset('sp_faces_normal_z_var'
                             , data=graph_sp["sp_faces_normal_z_var"], dtype='float32')
...
def read_spg(file_name):
    ...
    # graph["se_verticality_ratio"] = np.array(data_file["se_verticality_ratio"], dtype='float32')
    # graph["se_planarity_ratio"] = np.array(data_file["se_planarity_ratio"], dtype='float32')
    # graph["se_area_ratio"] = np.array(data_file["se_area_ratio"], dtype='float32')
    # graph["se_ele_ratio"] = np.array(data_file["se_ele_ratio"], dtype='float32')
    # graph["se_matrad_ratio"] = np.array(data_file["se_matrad_ratio"], dtype='float32')
...
def read_my_spg(file_name):
    ...
    graph["sp_ele"] = np.array(data_file["sp_ele"], dtype='float32')
    graph["sp_inmat_rad"] = np.array(data_file["sp_inmat_rad"], dtype='float32')
    graph["sp_circumference"] = np.array(data_file["sp_circumference"], dtype='float32')
    ...
    graph["sp_muls2_ele_0"] = np.array(data_file["sp_muls2_ele_0"], dtype='float32')
    graph["sp_muls2_ele_1"] = np.array(data_file["sp_muls2_ele_1"], dtype='float32')
    graph["sp_muls2_ele_2"] = np.array(data_file["sp_muls2_ele_2"], dtype='float32')
    graph["sp_pt2plane_dist_mean"] = np.array(data_file["sp_pt2plane_dist_mean"], dtype='float32')
    graph["sp_faces_normal_z_var"] = np.array(data_file["sp_faces_normal_z_var"], dtype='float32')

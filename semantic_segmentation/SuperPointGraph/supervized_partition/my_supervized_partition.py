...
from supervized_partition.my_graph_processing import *
from partition.my_provider import embedding2ply
from partition.my_provider import scalar2ply
from partition.my_provider import edge_class2ply2
from partition.my_provider import perfect_prediction
from partition.my_provider import write_spg
...
from my_folderhierarchy import FolderHierachy

def parse_args():
    parser = argparse.ArgumentParser(description='Large-scale Point Cloud Semantic Segmentation with Superpoint Graphs')
    # Dataset
    parser.add_argument('--dataset', default='custom_dataset', help='Dataset name: sema3d|s3dis|vkitti')
    #################### default = 1
    ...
    parser.add_argument('--db_train_name', default='train', help='Training set ()')
    parser.add_argument('--db_test_name', default='test', help='Test set ()')
    parser.add_argument('--ROOT_PATH', default='../datasets/custom_set_overseg')
    parser.add_argument('--odir', default='results_emb/', help='folder for saving the trained model')
    ...
    ####################Change param##########################default=10, test = 50, 100, 200, 300, 500, 1000, 10000, -1, 5000
    #test 4000, 3000, 2500, 2000,1500,1000, 500, 300, 100, 85, 70
    parser.add_argument('--CP_cutoff', default=55, type=int, help='Minimum accepted component size in cut pursuit. if negative, chose with respect tot his number and the reg_strength as explained in the paper')
    ...
    print(args.CP_cutoff)

    return args

def dataset(args):
        # Decide on the dataset
    dbinfo = get_custom_dataset_info(args)
    create_dataset = create_custom_datasets
...
def embed(args):
    ...
    train_dataset, test_dataset, validate_dataset = create_dataset(args)
    ...
    def evaluate(i_epoch):
        ...
        with torch.no_grad():
            ...
            loader = torch.utils.data.DataLoader(validate_dataset, batch_size=1, collate_fn=graph_collate,
            ...
            for bidx, (fname, edg_source, edg_target, is_transition, labels, objects, clouds_data, xyz) in enumerate(loader):
                ...
                if len(is_transition)>1:
                    ...
                    CM_classes.sp_count_predicted_batch(labels, per_pred)
                else:
                    loss = 0
    ...
    # iterate over dataset in batches
            for bidx, (fname, edg_source, edg_target, is_transition, labels, objects, clouds_data, xyz) in enumerate(loader):
                ...
                if len(is_transition)>1:
                    ...
                    confusion_matrix_classes.sp_count_predicted_batch(labels, per_pred)
                ...
                if args.spg_out:
                    ...
                    file_name = os.path.splitext(os.path.basename(fname[0]))[0] +'.h5'
                    spg_file = os.path.join(folder_hierarchy.spg_folder, file_name)

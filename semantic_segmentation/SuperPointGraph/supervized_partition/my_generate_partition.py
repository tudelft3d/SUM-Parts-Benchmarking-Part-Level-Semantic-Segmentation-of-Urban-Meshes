...
from supervized_partition import my_supervized_partition
from supervized_partition import my_graph_processing
from supervized_partition import losses
from partition import my_provider
from partition import my_graphs
...
def parse_args():
    ...
    parser.add_argument('--modeldir', default='../datasets/custom_set_overseg/results_emb/best')
    parser.add_argument('--cuda', default=0, type=int, help='Bool, use cuda')
    parser.add_argument('--input_folder',  type=str,
                        default='../datasets/custom_set_overseg/features_supervision/test')
    parser.add_argument('--output_folder', default='../datasets/custom_set_overseg/generated_partition', type=str, help='Folder that will contain the output')
...
def load_model(model_dir, cuda):
    checkpoint = torch.load(os.path.join(model_dir, my_supervized_partition.FolderHierachy.MODEL_FILE))
    training_args = checkpoint['args']
    training_args.cuda = cuda  # override cuda
    model = my_supervized_partition.create_model(training_args)
...
def get_dataloader(input_folder, training_args):
    ...
    dataset = tnt.dataset.ListDataset(file_list,
                                      functools.partial(my_graph_processing.graph_loader, train=False, args=training_args, db_path=""))
    loader = torch.utils.data.DataLoader(dataset, batch_size=1,
                                         collate_fn=my_graph_processing.graph_collate)
    return loader
...
def get_embedder(args):
    if args.learned_embeddings and args.ptn_embedding == 'ptn':
        ...
        ptnCloudEmbedder = my_graph_processing.spatialEmbedder(args)
    else:
...
def get_num_classes(args):
    ...
    if args.dataset == 's3dis':
        dbinfo = my_graph_processing.get_s3dis_info(args)
    elif args.dataset == 'sema3d':
        dbinfo = my_graph_processing.get_sema3d_info(args)
    elif args.dataset == 'vkitti':
        dbinfo = my_graph_processing.get_vkitti_info(args)
    else:
...
def process(data_tuple, model, output_folder, training_args, overwrite):
    ...
    graph_sp = my_graphs.compute_sp_graph(xyz, 100, pred_in_component, pred_components, labels, num_classes)

    my_provider.write_spg(spg_file, graph_sp, pred_components, pred_in_component)
...
def main():
    ...
    if not output_folder:
        ...
        output_folder = os.path.join(args.input_folder, '../..', my_supervized_partition.FolderHierachy.SPG_FOLDER)
    ...
    with torch.no_grad():
        process(dataloader, model, output_folder, training_args, args.overwrite)
